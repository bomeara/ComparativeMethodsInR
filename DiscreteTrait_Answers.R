...
one.cat.root.1 <- corHMM(primates$tree,primates$trait[,c(1,2)],rate.cat=1,node.states="marginal", ip=1, root.p=c(0,1))

#And equilibrium:
one.cat.root.maddfitz <- corHMM(primates$tree,primates$trait[,c(1,2)],rate.cat=1,node.states="marginal", ip=1, root.p="maddfitz")


#How does this affect rates?
print(one.cat)
print(one.cat.root.maddfitz)
print(one.cat.root.0)
print(one.cat.root.1)
